...
int main(int argc, char* argv[])
{
	if(argc!=11)
	{
		...
		std::cout << "Usage: " << argv[0] << " oldName newName vBond cBond kbend pos.x pos.y pos.z Umin radius\n";
		return 0;
	}
	...
	double vBond, cBond, kbend, Radius, Umin;
	std::stringstream cmdIn;
	for(int i=3;i<argc;i++)
		cmdIn << argv[i] << ' ';
	cmdIn >> vBond >> cBond >> kbend >> pos.x >> pos.y >> pos.z >> Umin >> Radius;
	...
	//pass the vertices and edges
	tesselaSphere<double> FirstSphere(sphereCenter, Radius, 3);
	FirstSphere.setcenterpointtype(5);
	FirstSphere.settype1(4);
	FirstSphere.settype2(5);
	FirstSphere.JanusRatioSet(0.5);
...
///initialize constants (force and potential constants)
	...
	//add the edge bonds between vertices and center point into the system
